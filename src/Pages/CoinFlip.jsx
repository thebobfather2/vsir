...
const CoinFlip = () => {
    ...
    const fetchCansTokens = useCallback(async () => {
        ...
        if (response.value.length === 0) {
            ...
            //begin filter for empty
            response.value.forEach((accountInfo, index) => {
                setCansBalance(accountInfo.account.data["parsed"]["info"]["tokenAmount"]["uiAmount"])
    ...
    const theFlip = () => {
        ...
        if (randomNumber === 1 && userSelection === 'heads') {
            setTimeout(function () { setFlipping(false); setIsWinner(true); setIsLoading(false); makeTX(); winSound() }, 3000)
        } else if (randomNumber === 0 && userSelection === 'tails') {
            setTimeout(function () { setFlipping(false); setIsWinner(true); setIsLoading(false); makeTX(); winSound() }, 3000)
        } else {
            setTimeout(function () { setFlipping(false); setIsWinner(false); setIsLoading(false); lossSound();fetchCansTokens(); fetchCarotTokens() }, 3000)
            setLossMessage(messageBank[Math.floor(Math.random() * messageBank.length)])
        }
    }
    ...
    const flipCoin = useCallback(async () => {
        ...
        if (userBet > 0) {
            ...
            try {
                ...
                setFlipping(false)
                setIsError(true)
                alert('An error occured.  Please check your wallet and try again.  If you feel this is a mistake, please open a ticket in Bobby Rabbits Discord')
            }
        }
    })
    return (
        <div className='CoinFlipMain'>
            {wallet ? (<>{isWinner &&
                <div className='bigWinnerMain' onClick={() => { setIsWinner(false); fetchCansTokens(); fetchCarotTokens() }}>
                    <Confetti
                    ...
                    <div className='winnerContainer'>
                        ...
                        {isLoading ? (<CircularProgress />) : (<>{rewardTX.length > 6 ?
                            ...
                            </Alert>) : rewardTX === 'false' ?
                                (<Alert severity="error">
                                    Error - Transaction was not confirmed-<strong>Please check wallet and try again</strong>
                                </Alert>) : (<div></div>)
                        }</>)}
                        <Button className='closeWinner' onClick={() => { setIsWinner(false); fetchCansTokens(); fetchCarotTokens() }}>Close</Button>
                    </div>
                </div>}
                <div className='balances'><div className='carotBalance'>$CAROT Balance: {balance}</div><div className='carotBalance'>$CANS Balance: {cansBalance}</div></div>
                ...
                <div className='choices'>
                    {(userSelection === 'heads') ? (<><Button className='flipchosen'>Heads</Button>
                        ...
                        <Button className='flip' onClick={() => { setUserSelection("heads") }}>Heads</Button>
                ...
                <div className='betArray'>
                    <Button className={userBet === 1 ? 'betsChosen' : 'bets'} onClick={() => { setUserBet(1) }}>1 $CAROT</Button>
                    <Button className={userBet === 5 ? 'betsChosen' : 'bets'} onClick={() => { setUserBet(5) }}>5 $CAROT</Button>
                    <Button className={userBet === 10 ? 'betsChosen' : 'bets'} onClick={() => { setUserBet(10) }}>10 $CAROT</Button>
                </div>
                <br></br>
                {isTokenAccount ? (<>{!isLoading ? (<Button size='large' className='theFlipper' onClick={flipCoin} disabled={!(userSelection === 'heads' || userSelection === 'tails') || (userBet === null) || (balance < 1)}>Flip</Button>) : (<Button size='large' className='theFlipper'><CircularProgress /></Button>)}</>)
                    : (<>{!isLoading ? (<Button size='large' className='theFlipper' onClick={createAccount}>Create Reward Account</Button>) : (<Button size='large' className='theFlipper'><CircularProgress /></Button>)}</>)}
                <br></br>
                {(isWinner === null) ? (<div className='Loser'></div>)
                    :
                    (<>
                        {isWinner ?
            ...
            </>) : (<><h2>Please Connect Wallet</h2><br></br><WalletMultiButton /></>)}</div>
    )
}
